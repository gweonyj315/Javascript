const data = [
  ...
  const makeCard = (x)=>`
            <div class="card">
                <img src="${x.image}" alt="">
                <div class="info">
                    <div class="name">${x.firstName} + " " + ${maidenName} + " " + ${lastName}</div>
                    <div class="date">${x.birthDate}</div>
                    <div class="department">${x.department}</div>
                    <div class="mail">${x.email}</div>
                </div>
            </div>`;
data.forEach((x)=>{
    box.insertAdjacentHTML("beforeend", makeCard(x));
})